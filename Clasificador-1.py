...
with open('base.json','r') as read_json:
	infoJson = json.load(read_json)
	probabilidades = infoJson['Probabilidades']
f = open ('tweet.txt','r')
mensaje = f.read()
a = ''
pila = []
for  i in mensaje:
    if i != ' ':
        ...
        Elemento = [ e for e in probabilidades if e[0] == a]
        if Elemento != False and Elemento != []:
            pila.append(Elemento[0][1])
        a = ''
suma = 0
if len(pila) >= 1:
    for j in pila:
